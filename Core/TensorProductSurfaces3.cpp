...
GLboolean TensorProductSurface3::RenderDataSides(GLenum render_mode, int side) const
{
    if(!_vbo_data)
        return GL_FALSE;
    ...
    glEnableClientState(GL_VERTEX_ARRAY);
        ...
        if(side == 0)
               ...
               if(side == 3)
               {
                   ...
                   glDrawArrays(render_mode,4,1);
                   glDrawArrays(render_mode,8,1);
                   glDrawArrays(render_mode,12,1);
                   glPointSize(1.0f);
               ...
               if(side == 2)
               {
                   ...
                   glDrawArrays(render_mode,12,4);
                   glDrawArrays(render_mode,13,1);
                   glDrawArrays(render_mode,14,1);
                   glDrawArrays(render_mode,15,1);
                   glPointSize(1.0f);
               }
               if(side == 1)
               {
                   ...
                   glDrawArrays(render_mode,15,1);
                   glDrawArrays(render_mode,11,1);
                   glDrawArrays(render_mode,7,1);
                   glDrawArrays(render_mode,3,1);
                   glPointSize(1.0f);
                   int offset =0;
                   for (GLuint r = 0; r < _data.GetRowCount(); ++r) {
...
GLboolean TensorProductSurface3::RenderDataCustomized(GLenum render_mode, int col, int row) const
{
    int highlighted_point = 4*row+col;
    if(!_vbo_data)
        return GL_FALSE;
    ...
    glEnableClientState(GL_VERTEX_ARRAY);
        ...
        glDrawArrays(render_mode, highlighted_point, 1);
        glPointSize(10.0f);
        glColor3f(0.0f,1.0f,1.0f);
          GLuint offset = 0;
          for (GLuint r = 0; r < _data.GetRowCount(); ++r) {
